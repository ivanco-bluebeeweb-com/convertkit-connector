...
async def connect_convertkit_connector(params: ConnectParams, ctx) -> ActionResult:
    ...
    return ActionResult.ok(rec, summary=f"Connected ConvertKit ({rec['label']}).")
...
async def disconnect_convertkit_connector(params: ConnectionIdParams, ctx) -> ActionResult:
    ...
    if params.connection_id:
        ...
        conns.clear()
    await _save_conns(ctx, conns)
    return ActionResult.ok({"success": True, "message": "Disconnected successfully."}, summary="Disconnected ConvertKit connection.")
