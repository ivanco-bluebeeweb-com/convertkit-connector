"""Resource handlers for ConvertKit Connector."""
...
@chat.function("list_subscribers", "List subscribers in ConvertKit.", action_type="read", chain_callable=True, event="convertkit-connector.list_subscribers", effects=["read:subscribers"], data_model=SubscriberList)
...
@chat.function("get_subscriber", "Get details of one Subscriber in ConvertKit.", action_type="read", chain_callable=True, event="convertkit-connector.get_subscriber", effects=["read:subscriber"], data_model=SubscriberRecord)
...
@chat.function("audit_subscriber_health", "Audit health of ConvertKit subscribers and connectivity.", action_type="read", chain_callable=True, event="convertkit-connector.audit_subscriber_health", effects=["read:audit"], data_model=AuditHealthReport)
async def audit_subscriber_health(params: ConnectionIdParams, ctx) -> ActionResult:
    ...
    try:
        ...
        return ActionResult.success({
            ...
            "summary": f"ConvertKit healthy. Sampled {len(items)} subscribers."
        }, summary=f"ConvertKit health check passed with {len(items)} subscribers.")
    except Exception as e:
        return ActionResult.error(f"Error auditing ConvertKit health: {e}")
